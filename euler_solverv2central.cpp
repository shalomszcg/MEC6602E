...
private:
    ...
    // --- NEW: Artificial Dissipation (JST) Members ---
...
public:
    ...
    // --- MODIFIED: Also computes cell-centered spectral radius ---
    void computeTimeStep() {
        ...
        for (int jj = NGHOST; jj < mesh.njc + NGHOST; jj++) {
            for (int ii = NGHOST; ii < mesh.nic + NGHOST; ii++) {
                ...
                // --- NEW: Store spectral radius for dissipation ---
                spectral_radius_cell[idx] = V_mag + a;
                
                double lambda_max = spectral_radius_cell[idx]; // Use stored value
    ...
    void addArtificialDissipation() {
        ...
        for (int jj = NGHOST; jj < mesh.njc + NGHOST; jj++) {
            for (int ii = NGHOST; ii < mesh.nic + NGHOST; ii++) {
                ...
                // R_new = R_conv - (D_i+1/2 - D_i-1/2) - (D_j+1/2 - D_j-1/2)
                // Note: D_i[idx] is D at i+1/2. D_i[idx_im1] is D at i-1/2.
                // Note: D_j[idx] is D at j+1/2. D_j[idx_jm1] is D at j-1/2.
